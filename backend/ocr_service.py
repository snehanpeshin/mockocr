...
def _bedrock_system_prompt() -> str:
    return (
        ...
        "For math and science, transcribe exactly what is visible. Never replace an equation "
        "with a known formula, theorem, identity, explanation, or solution. "
...
def _bedrock_user_prompt(
    ...
    return (
        ...
        "Treat OCR as a draft, not final truth, but the image is the authority. If an image is "
        "attached, compare the OCR draft against the visual page before finalizing. Build a "
        "page-level symbol inventory, then fix only visually and contextually supported character "
        "substitutions using neighboring lines, repeated usage, grammar, and the image. "
...
def _math_symbol_verification_prompt() -> str:
    return (
        "Mathematical transcription rules: enabled.\n"
        "Copy every equation as written. Never replace an equation with a known formula. "
        "Never infer missing variables, symbols, exponents, operators, constants, formula names, "
        "theorem names, or headings. Never convert one mathematical identity into another. "
        "Do not name formulas, classify formulas, explain formulas, solve equations, simplify "
        "expressions, or add mathematical context unless that wording is explicitly visible. "
        "Every equation must be traceable character-for-character to the image or OCR draft. "
        "Use mathematical knowledge only as a guardrail to avoid obvious OCR glyph mistakes, not "
        "as a source of new content. A correction is allowed only when the symbol is visually "
        "ambiguous and the same symbol is clearly repeated elsewhere on the page. "
        "For likely glyph confusions such as b/6, B/8, O/0, o/0, l/1, I/1, S/5, z/2, x/×, "
        "plus/t, minus/dash, and handwritten Greek/Latin lookalikes, preserve uncertainty with "
        "[?] unless the image and page-level symbol inventory strongly support one reading. "
        ...
        "visible in the image or OCR draft. Do not create section titles such as 'Law of Cosines' "
        "or 'Difference of Squares' unless those exact words are visible. "
        "When you make a conservative symbol correction, add a brief 'Corrections made' section "
        "after the transcription. Explain only OCR-level corrections, not mathematical identities. "
        "If no meaningful correction was made, omit that section. If more than one interpretation "
        "remains plausible, include it under 'Possible OCR Ambiguities'.\n"
    )
